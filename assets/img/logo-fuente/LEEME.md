...
No hay versión en negativo (blanca) con título "ASESORES" ni "MARKETING" entregada por el cliente — por eso el footer de Asesoría sigue usando el isotipo blanco sin título (`logo-a360-blanco.png`).

**`logo-marketing-blanco.png`** (en `/assets/img/`, no en esta carpeta de fuentes porque no es un archivo del cliente): generado a partir de `logo-marketing.png` para el footer de Marketing, que sí pedía el título "MARKETING" en blanco. Se generó por script, replicando la misma técnica que ya usa `logo-a360-blanco.png` para el icono: cada píxel naranja/azul/marino pasa a blanco opaco, y cada píxel que ya era blanco en el original (el hueco del arco y de los dígitos "360", recortados sobre el naranja) pasa a transparente — así el hueco se sigue viendo en vez de desaparecer, que es lo que pasaba con un filtro tipo `brightness(0) invert(1)` (aplana todo a un blanco plano sin huecos). Si el cliente entrega el negativo oficial con título, sustituir este archivo (mismo nombre y tamaño 973×1101).
